...
async function getUserAccountsStatus(query) {
...
async function editUserAccountStatus(payload) {
  const response = await api.post(
...
const useUserAccountStatus = commonCrudFactory({
  key: 'user-account-status',
  interval: 300000, //3000 sec
  api: {
    getItems: getUserAccountsStatus,
    editItem: editUserAccountStatus,
  },
});

export default useUserAccountStatus;
